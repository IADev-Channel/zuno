...
describe("milestone 9 subscription protocol", () => {
	...
	it("negotiates legacy and subscription-aware protocols", () => {
		expect(negotiateZunoProtocol(1)).toEqual({
			version: 1,
			subscriptions: true,
		});
		expect(negotiateZunoProtocol(99)).toEqual({
			version: 0,
			subscriptions: false,
		});
	});
	it("rejects partition and topic escalation plus connection limits", () => {
		const forbiddenPartition = createZunoSubscription({
			id: "1",
			partition: "tenant-b",
			topic: "cart",
		});
		expect(validateSubscriptions([forbiddenPartition], principal).reason).toBe(
			"PARTITION_FORBIDDEN",
		);
		const forbiddenTopic = createZunoSubscription({
			id: "2",
			partition: "tenant-a",
			topic: "admin",
		});
		expect(validateSubscriptions([forbiddenTopic], principal).reason).toBe(
			"TOPIC_FORBIDDEN",
		);
		const allowed = createZunoSubscription({
			id: "3",
			partition: "tenant-a",
			topic: "cart",
		});
		expect(
			validateSubscriptions([allowed], principal, {
				maxSubscriptionsPerConnection: 0,
			}).reason,
		).toBe("SUBSCRIPTION_LIMIT_EXCEEDED");
	});
	it("rejects cross-tenant mutations before state access", () => {
		const errors = authorizeStateEvent(
			{ storeKey: "tenant-b:cart:1", state: {} },
			principal,
		);
	...
	it("supports subscription churn without stale delivery", () => {
		...
		const sub = createZunoSubscription({
			id: "churn",
			partition: "tenant-a",
			topic: "cart",
		});
	...
	it("scopes live delivery, replay, and snapshots", () => {
		...
		const cart = server.compareAndSet({
			storeKey: "tenant-a:cart:1",
			state: { n: 2 },
			baseVersion: 1,
		});
		const admin = server.compareAndSet({
			storeKey: "tenant-a:admin:1",
			state: { secret: true },
		});
		...
		expect(
			server.getScopedEventsAfter(0, "tenant-a", new Set(["cart"])),
		).toHaveLength(2);
		expect(
			Object.keys(server.getScopedUniverseState("tenant-a", new Set(["cart"]))),
		).toEqual(["tenant-a:cart:1"]);
	});
});
