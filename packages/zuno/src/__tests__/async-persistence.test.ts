...
describe("async persistence compatibility", () => {
	it("lifts the synchronous persistence contract without changing CAS semantics", async () => {
		...
		const first = await persistence.compareAndSet(
			{
		...
		const duplicate = await persistence.compareAndSet(
			{
				storeKey: "tenant:cart:1",
				state: { quantity: 999 },
				baseVersion: 0,
		...
		const stale = await persistence.compareAndSet(
			{
				storeKey: "tenant:cart:1",
				state: { quantity: 2 },
				baseVersion: 0,
		...
		expect(await persistence.getRecord("tenant:cart:1")).toEqual({
			state: { quantity: 1 },
			version: 1,
		});
		expect((await persistence.readEvents({ afterEventId: 0 })).length).toBe(1);
	});
